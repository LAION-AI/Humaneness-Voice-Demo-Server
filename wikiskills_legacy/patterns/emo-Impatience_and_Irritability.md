...
`emo/Impatience_and_Irritability` — emotion. Generated from the measured JSON on 2026-09-02 by `code/build_wikiskills.py`; do not edit by hand, edit the generator or the measurements it reads.

Steering vector key `emo:Impatience_and_Irritability`. Target metric `emo_pct`, baseline 0.277 over 10 prompts.

## Coefficient block

```yaml
attribute: emo/Impatience_and_Irritability
steering_key: emo:Impatience_and_Irritability
target_metric: emo_pct
adapter:
  name: sft3_emotion:Impatience_and_Irritability
  usable: false
  safe_w: null
  strong_w: null
...
balanced:
  mode: adapter+steer
  lora: {name: "sft3_emotion:Impatience_and_Irritability", w: 1.0}
  steer:
    - {key: "emo:Impatience_and_Irritability", alpha: 0.1, taps: top1}   # h19
  steer_branch: cond
  cfg: {g: 1.0}
  measured: {d_target: +0.195, t: 3.56, n_prompts: 10, n_up: 10,
             d_wer_parakeet: +0.004, d_genuineness: -0.268,
             d_blend: -0.332, d_r_burst: -0.095, d_dur_err_abs_s: +0.003}
...
high_effect:
  ...
  steer_branch: cond
  cfg: {g: 2.0}
  measured: {d_target: +0.366, t: 5.71, n_prompts: 10, n_up: 10,
             d_wer_parakeet: +0.021, d_genuineness: -0.619,
             d_blend: -0.665, d_r_burst: -0.109, d_dur_err_abs_s: -0.004}
  beats_random_floor: true
```
...
Mode **`adapter+steer`**. Adapter `sft3_emotion:Impatience_and_Irritability` at w = 1.0. Steering on the cond branch. 

Target moves **+0.195** (t 3.56, better on 10 of 10 prompts), from 0.277 to 0.472. This clears the matched random-direction floor of -0.033.

| guardrail | baseline | at this point | change |
|---|--:|--:|--:|
| word error (Parakeet) | 0.077 | 0.081 | +0.004 |
| genuineness, raw of 6 | 3.873 | 3.606 | -0.268 |
| burst blend, raw of 10 | 5.212 | 4.880 | -0.332 |
| burst realisation | 0.472 | 0.376 | -0.095 |
| |duration error|, s | 0.051 | 0.054 | +0.003 |

## High effect operating point

Mode **`steer+cfg`**. No adapter. Guidance g = 2.0. Steering on the cond branch. 

Target moves **+0.366** (t 5.71, better on 10 of 10 prompts), from 0.277 to 0.644. This clears the matched random-direction floor of -0.033.

| guardrail | baseline | at this point | change |
|---|--:|--:|--:|
| word error (Parakeet) | 0.077 | 0.098 | +0.021 |
| genuineness, raw of 6 | 3.873 | 3.254 | -0.619 |
| burst blend, raw of 10 | 5.212 | 4.546 | -0.665 |
| burst realisation | 0.472 | 0.362 | -0.109 |
| |duration error|, s | 0.051 | 0.047 | -0.004 |
...
|--:|--:|--:|--:|--:|--:|--:|--:|:--|
| 0.25 | 0.253 | -0.007 | -0.28 | 4/10 | +0.019 | +0.040 | +0.029 | yes |
| 0.5 | 0.263 | +0.003 | 0.12 | 5/10 | +0.043 | -0.062 | +0.005 | yes |
| 0.75 | 0.284 | +0.024 | 1.23 | 7/10 | -0.003 | -0.075 | +0.006 | yes |
| 1.0 | 0.308 | +0.048 | 2.15 | 7/10 | -0.010 | -0.073 | -0.015 | yes |
| 1.25 | 0.293 | +0.033 | 1.79 | 6/10 | +0.015 | -0.211 | -0.127 | yes |
| 1.5 | 0.317 | +0.057 | 1.94 | 8/10 | +0.027 | -0.255 | -0.118 | yes |
...
| pair | interaction | t |
|---|--:|--:|
| adapter × steering | +0.025 | 0.15 |
| adapter × guidance | -0.120 | -0.91 |
| steering × guidance | +0.491 | 3.09 |

Target in SD units at each corner of the 2×2×2 (bits are adapter, steering, guidance):

| 000 | 001 | 010 | 011 | 100 | 101 | 110 | 111 |
|--:|--:|--:|--:|--:|--:|--:|--:|
| 0.893 | 0.983 | 1.394 | 2.072 | 0.897 | 0.964 | 1.520 | 1.980 |
...
* Recipes and every Δ in this page: `combination-study/stats/comb_recommendations.json`, key `emo/Impatience_and_Irritability`.
* Interaction terms: `combination-study/stats/analysis.json`, `t2`.
* Dose ladder: `lora-dose/coefficients.json`, key `emotion/Impatience_and_Irritability`.
* Layer ranking behind `topK`: `work_vb/tap_rank.json`.
* Steering vectors: `actforensics/vectors/p3_vectors_ext.npz`, row `emo:Impatience_and_Irritability`.
