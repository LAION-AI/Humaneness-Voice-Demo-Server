...
# Hesitation sounds, in both languages the demo speaks, plus the spellings the
# recogniser reaches for.  They are collapsed to one token before word error is
# computed: the prompt asks the director to use them, and the recogniser writes
# whichever it likes -- "ehm" comes back as "um", "ähm" as "um", "öh" as "oh" --
# so counting them as errors would charge the director for following the prompt
# and push the best-of-N reward away from the takes that hesitate like a person.
FILLERS = {
    "uh", "uhm", "um", "umm", "ehm", "eh", "ehh", "er", "erm", "err",
    "hm", "hmm", "hmmm", "mhm", "mm", "mmm", "ah", "ahh", "oh", "ooh",
    "öh", "öhm", "äh", "ähm", "ah", "na", "tja",
}
_FILLER = "\u2039f\u203a"          # one token every filler maps onto


def words(s, fold_fillers=True):
    """Words, lower-cased.  Every hesitation sound folds to one token."""
    # The recogniser writes "weiß" where a script wrote "weiss"; that is a
    # spelling difference, not a misread word, and it was charging German lines
    # a word error the speech model never made.
    ws = _W.findall(str(s).lower().replace("\u00df", "ss"))
    if not fold_fillers:
        return ws
    return [_FILLER if w in FILLERS else w for w in ws]
