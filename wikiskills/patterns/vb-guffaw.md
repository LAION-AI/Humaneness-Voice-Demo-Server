...
| größengleiche Zufallskontrolle | 0,006 |
| netto über der Zufallskontrolle | 0,061 |
| Genuineness (nur berichtet, kein Tor) | 2,16 |
| WER (Parakeet), absolut | 0,413 |
| WER gepaart gegen die eigene w = 0-Zelle | 0,093 (Grenze +0,104) |

Dieselbe Klasse, die drei Wege nebeneinander (jeweils die beste Zelle unter w ≤ 1,5):

| Weg | Adapter | Form / w | Gruppe | streng |
|---|---|---|--:|--:|
| eigener Adapter | `percls_alone` | solo / 1,00 | **0,067** | 0,000 |
| Gruppen-Adapter (gepoolt) | `grpfull` | inline / 0,50 | **0,000** | 0,000 |

## Warum die Zahl so ist

Die Klasse bleibt unter 0,15. Der Detektor sieht sie (Boden nicht gemessen), das Modell erzeugt sie aber selten. Best-of-N mit 34 Kandidaten ist der einzige wirksame Hebel.

## Wenn es nicht geht

Wenn dieses Etikett nicht trifft: die Gruppe `laugh_loud` enthält `cackle`. Der Adapter des stärksten gemessenen Mitglieds — `vb/cackle` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,200).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `guffaw` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/guffaw) |
| eigener Adapter, nur Erstwahl-Zeilen | `guffaw` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class_top1/guffaw) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `guffaw` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/guffaw) |
| Gruppen-Adapter, volle Dosis — Gruppe `laugh_loud` | `laugh_loud` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_full/laugh_loud) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (guffaw) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
