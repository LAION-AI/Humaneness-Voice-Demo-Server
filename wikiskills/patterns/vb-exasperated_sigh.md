...
| Maß | Wert |
|---|--:|
| Trefferquote, **streng** (exakt dieselbe Klasse) | **0,233** |
| Trefferquote, **Gruppe** (irgendein Mitglied) | **0,367** |
| größengleiche Zufallskontrolle | 0,286 |
| netto über der Zufallskontrolle | 0,080 |
| Genuineness (nur berichtet, kein Tor) | 2,30 |
| WER (Parakeet), absolut | 0,142 |
| WER gepaart gegen die eigene w = 0-Zelle | -0,079 (Grenze +0,104) |
| **Detektor-Boden**, echte Sprache, streng | 0,226 |

Dieselbe Klasse, die drei Wege nebeneinander (jeweils die beste Zelle unter w ≤ 1,5):

| Weg | Adapter | Form / w | Gruppe | streng |
|---|---|---|--:|--:|
| eigener Adapter | `shipped` | inline / 1,50 | **0,233** | 0,133 |
| geliehen vom Gruppenmitglied | `bestmem` | inline / 1,50 | **0,333** | 0,100 |
| Gruppen-Adapter (gepoolt) | `grp25` | inline / 1,00 | **0,367** | 0,233 |

Geliehen gegen eigen, gepaart bei gleicher Form und gleichem Gewicht: d = 0,2333, t = 2,09, n = 10.

## Warum die Zahl so ist

**Die Grenze ist hier das Messgerät, nicht das Modell.** Der Produktionsdetektor erkennt diese Klasse auf echter Sprache nur zu 0,226. Eine gemessene Trefferquote wesentlich über 0,226 ist deshalb gar nicht erreichbar, wie gut der Adapter auch sein mag. Die Zahl oben ist eine Untergrenze für das, was tatsächlich erzeugt wird.

## Wenn es nicht geht

Wenn dieses Etikett nicht trifft: die Gruppe `sigh_neg` enthält `displeased_grunt`, `exhausted_groan`, `frustrated_groan`. Der Adapter des stärksten gemessenen Mitglieds — `vb/exhausted_groan` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,467).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `exasperated_sigh` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/exasperated_sigh) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `exasperated_sigh` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/exasperated_sigh) |
| Gruppen-Adapter, volle Dosis — Gruppe `sigh_neg` | `sigh_neg` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_full/sigh_neg) |
| Gruppen-Adapter, 25 % synthetisch — Gruppe `sigh_neg` | `sigh_neg` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_dose25/sigh_neg) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (exasperated sigh) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
