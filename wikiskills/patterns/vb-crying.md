...
> Stand 5. September 2026 · Gruppe `wail` im 23-Gruppen-Schema · **nicht einzeln gemessen** · eigener Adapter vorhanden: nein
>
> Gruppen-Mitglieder: `crying`, `mournful_wail`

## Rezept

Diese Bezeichnung wurde **nicht einzeln gemessen**. Sie gehört zur Gruppe `wail`; dort sind gemessen: `mournful_wail`.


Wer diesen Laut braucht, nimmt den Adapter des stärksten gemessenen Geschwisters, `vb/mournful_wail`, mit der dort belegten Einstellung — siehe `vb-mournful_wail.md` (dort gemessen: 0,033 auf Gruppenebene).
...
Wenn dieses Etikett nicht trifft: die Gruppe `wail` enthält `mournful_wail`. Der Adapter des stärksten gemessenen Mitglieds — `vb/mournful_wail` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,033).
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (crying) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
