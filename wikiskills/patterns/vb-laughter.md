...
> Stand 5. September 2026 · Gruppe `laugh_loud` im 23-Gruppen-Schema · **nicht einzeln gemessen** · eigener Adapter vorhanden: nein
>
> Gruppen-Mitglieder: `cackle`, `guffaw`, `laugh`, `laughter`

## Rezept

Diese Bezeichnung wurde **nicht einzeln gemessen**. Sie gehört zur Gruppe `laugh_loud`; dort sind gemessen: `cackle`, `guffaw`.


Wer diesen Laut braucht, nimmt den Adapter des stärksten gemessenen Geschwisters, `vb/cackle`, mit der dort belegten Einstellung — siehe `vb-cackle.md` (dort gemessen: 0,200 auf Gruppenebene).
...
Wenn dieses Etikett nicht trifft: die Gruppe `laugh_loud` enthält `cackle`, `guffaw`. Der Adapter des stärksten gemessenen Mitglieds — `vb/cackle` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,200).
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (laughter) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
