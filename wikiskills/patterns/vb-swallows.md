...
> Stand 5. September 2026 · Gruppe `swallow` im 23-Gruppen-Schema · gemessen · eigener Adapter vorhanden: ja
>
...
| größengleiche Zufallskontrolle | 0,037 |
| netto über der Zufallskontrolle | -0,037 |
| Genuineness (nur berichtet, kein Tor) | 2,44 |
| WER (Parakeet), absolut | 0,129 |
| WER gepaart gegen die eigene w = 0-Zelle | -0,045 (Grenze +0,104) |
...
Nichts an diesem Etikett funktioniert. Die ehrliche Empfehlung ist, den Laut **nicht über das Burst-Etikett** anzufordern, sondern den Charakter über die Emotionsanweisung zu steuern.
 In derselben Gruppe `swallow` sind gemessen: `gulps` — dort lohnt der Blick.
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `swallows` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/swallows) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `swallows` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/swallows) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (swallows) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
