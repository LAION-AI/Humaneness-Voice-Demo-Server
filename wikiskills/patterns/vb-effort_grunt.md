...
| | |
|---|---|
| Adapter | Gruppen-Adapter, 25 % DramaBox (nur der Adapter, ohne Produktions-Stapel) |
| Gewicht | **1,00** |
| Skriptart | **eingebettet (inline)** |
| Best-of-N | **17** Kandidaten für 90 % (konservativ, eine Seed-Rausch-Standardabweichung abgezogen: 35) |
...
| Trefferquote, **Gruppe** (irgendein Mitglied) | **0,133** |
| größengleiche Zufallskontrolle | 0,010 |
| netto über der Zufallskontrolle | 0,124 |
| Genuineness (nur berichtet, kein Tor) | 2,21 |
| WER (Parakeet), absolut | 0,099 |
| WER gepaart gegen die eigene w = 0-Zelle | -0,058 (Grenze +0,104) |
...
| geliehen vom Gruppenmitglied | `bestmem` | inline / 1,00 | **0,100** | 0,000 |
| Gruppen-Adapter (gepoolt) | `grp25_alone` | inline / 1,00 | **0,133** | 0,000 |

Geliehen gegen eigen, gepaart bei gleicher Form und gleichem Gewicht: d = 0,0000, t = —, n = 10.

## Warum die Zahl so ist

Die Klasse bleibt unter 0,15. Der Detektor sieht sie (Boden nicht gemessen), das Modell erzeugt sie aber selten. Best-of-N mit 17 Kandidaten ist der einzige wirksame Hebel.

## Wenn es nicht geht

Wenn dieses Etikett nicht trifft: die Gruppe `grunt` enthält `affirmative_grunt`. Der Adapter des stärksten gemessenen Mitglieds — `vb/effort_grunt` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,367).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `effort_grunt` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/effort_grunt) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `effort_grunt` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/effort_grunt) |
| Gruppen-Adapter, volle Dosis — Gruppe `grunt` | `grunt` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_full/grunt) |
| Gruppen-Adapter, 25 % synthetisch — Gruppe `grunt` | `grunt` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_dose25/grunt) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (effort grunt) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
