...
`emo/Jealousy_and_Envy` — emotion. Generated from the measured JSON on 2026-09-02 by `code/build_wikiskills.py`; do not edit by hand, edit the generator or the measurements it reads.

Steering vector key `emo:Jealousy_and_Envy`. Target metric `emo_pct`, baseline 0.502 over 10 prompts.

## Coefficient block

```yaml
attribute: emo/Jealousy_and_Envy
steering_key: emo:Jealousy_and_Envy
target_metric: emo_pct
adapter:
  name: sft3_emotion:Jealousy_and_Envy
  usable: false
  safe_w: null
  strong_w: null
...
balanced:
  ...
  lora: null
  steer:
    - {key: "emo:Jealousy_and_Envy", alpha: 0.1, taps: top1}   # h19
    - {key: "emo:Emotional_Numbness", alpha: -0.1, taps: top1}   # h20
  steer_branch: cond
  cfg: {g: 2.0}
  measured: {d_target: +0.145, t: 2.98, n_prompts: 10, n_up: 9,
             d_wer_parakeet: +0.005, d_genuineness: -0.006,
             d_blend: +0.569, d_r_burst: -0.080, d_dur_err_abs_s: -0.004}
...
Target moves **+0.145** (t 2.98, better on 9 of 10 prompts), from 0.502 to 0.647. This clears the matched random-direction floor of +0.027.

| guardrail | baseline | at this point | change |
|---|--:|--:|--:|
| word error (Parakeet) | 0.089 | 0.094 | +0.005 |
| genuineness, raw of 6 | 3.795 | 3.789 | -0.006 |
| burst blend, raw of 10 | 4.564 | 5.133 | +0.569 |
| burst realisation | 0.481 | 0.402 | -0.080 |
| |duration error|, s | 0.061 | 0.057 | -0.004 |
...
|--:|--:|--:|--:|--:|--:|--:|--:|:--|
| 0.25 | 0.398 | +0.015 | 0.30 | 5/10 | -0.002 | +0.080 | -0.066 | yes |
| 0.5 | 0.425 | +0.043 | 0.59 | 5/10 | +0.021 | +0.026 | -0.104 | yes |
| 0.75 | 0.392 | +0.009 | 0.16 | 4/10 | -0.001 | +0.065 | -0.049 | yes |
| 1.0 | 0.394 | +0.012 | 0.24 | 5/10 | +0.017 | +0.185 | -0.016 | yes |
| 1.25 | 0.434 | +0.052 | 1.38 | 7/10 | +0.012 | +0.159 | +0.026 | yes |
| 1.5 | 0.437 | +0.055 | 0.74 | 5/10 | +0.014 | +0.112 | -0.057 | yes |
...
| pair | interaction | t |
|---|--:|--:|
| adapter × steering | -0.134 | -1.11 |
| adapter × guidance | -0.216 | -0.93 |
| steering × guidance | +0.229 | 0.89 |

Target in SD units at each corner of the 2×2×2 (bits are adapter, steering, guidance):

| 000 | 001 | 010 | 011 | 100 | 101 | 110 | 111 |
|--:|--:|--:|--:|--:|--:|--:|--:|
| 2.290 | 2.352 | 2.542 | 2.768 | 2.435 | 2.216 | 2.488 | 2.563 |
...
* Recipes and every Δ in this page: `combination-study/stats/comb_recommendations.json`, key `emo/Jealousy_and_Envy`.
* Interaction terms: `combination-study/stats/analysis.json`, `t2`.
* Dose ladder: `lora-dose/coefficients.json`, key `emotion/Jealousy_and_Envy`.
* Layer ranking behind `topK`: `work_vb/tap_rank.json`.
* Steering vectors: `actforensics/vectors/p3_vectors_ext.npz`, row `emo:Jealousy_and_Envy`.
