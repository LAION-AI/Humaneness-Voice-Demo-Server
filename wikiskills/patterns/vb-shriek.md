...
> Stand 5. September 2026 · Gruppe `scream` im 23-Gruppen-Schema · gemessen · eigener Adapter vorhanden: ja
>
> Gruppen-Mitglieder: `scream`, `screaming`, `screech`, `shout`, `shriek`, `yell`
...
| größengleiche Zufallskontrolle | 0,025 |
| netto über der Zufallskontrolle | 0,408 |
| Genuineness (nur berichtet, kein Tor) | 2,00 |
| WER (Parakeet), absolut | 0,357 |
| WER gepaart gegen die eigene w = 0-Zelle | 0,023 (Grenze +0,104) |

Dieselbe Klasse, die drei Wege nebeneinander (jeweils die beste Zelle unter w ≤ 1,5):

| Weg | Adapter | Form / w | Gruppe | streng |
|---|---|---|--:|--:|
| eigener Adapter | `percls_alone` | solo / 1,50 | **0,433** | 0,000 |
| geliehen vom Gruppenmitglied | `bestmem_alone` | solo / 1,50 | **0,333** | 0,000 |
| Gruppen-Adapter (gepoolt) | `grpfull_alone` | inline / 1,00 | **0,400** | 0,000 |

Geliehen gegen eigen, gepaart bei gleicher Form und gleichem Gewicht: d = -0,1000, t = -0,76, n = 10.
...
Wenn dieses Etikett nicht trifft: die Gruppe `scream` enthält `scream`. Der Adapter des stärksten gemessenen Mitglieds — `vb/scream` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,600).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `shriek` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/shriek) |
| eigener Adapter, nur Erstwahl-Zeilen | `shriek` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class_top1/shriek) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `shriek` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/shriek) |
| Gruppen-Adapter, volle Dosis — Gruppe `scream` | `scream` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_full/scream) |
| Gruppen-Adapter, 25 % synthetisch — Gruppe `scream` | `scream` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_dose25/scream) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (shriek) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
