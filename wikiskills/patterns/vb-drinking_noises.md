...
> Stand 5. September 2026 · Gruppe `swallow` im 23-Gruppen-Schema · **nicht einzeln gemessen** · eigener Adapter vorhanden: ja
>
> Gruppen-Mitglieder: `drinking_noises`, `gulp`, `gulps`, `nervous_gulp`, `slurping_noises`, `swallow`, `swallows`

## Rezept

Diese Bezeichnung wurde **nicht einzeln gemessen**. Sie gehört zur Gruppe `swallow`; dort sind gemessen: `gulps`, `swallows`.


In dieser Gruppe funktioniert **nichts**: das gemessene Mitglied kommt nicht über 0,000. Für diesen Laut gibt es kein Rezept, das etwas bringt — siehe `vb-gulps.md`.
...
Die Gruppe `swallow` hilft hier nicht: auch ihre gemessenen Mitglieder (`gulps`, `swallows`) kommen nicht über 0,000. Für diesen Laut gibt es derzeit **kein funktionierendes Rezept**; den Charakter über die Emotionsanweisung zu steuern ist der ehrliche Rat.
...
|---|---|---|
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `drinking_noises` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/drinking_noises) |
| Gruppen-Adapter, volle Dosis — Gruppe `swallow` | `swallow` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_full/swallow) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (drinking noises) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
