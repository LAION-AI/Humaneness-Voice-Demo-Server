...
| größengleiche Zufallskontrolle | 0,477 |
| netto über der Zufallskontrolle | 0,323 |
| Genuineness (nur berichtet, kein Tor) | 2,37 |
| WER (Parakeet), absolut | 0,106 |
| WER gepaart gegen die eigene w = 0-Zelle | -0,051 (Grenze +0,104) |
| **Detektor-Boden**, echte Sprache, streng | 0,469 |

Dieselbe Klasse, die drei Wege nebeneinander (jeweils die beste Zelle unter w ≤ 1,5):

| Weg | Adapter | Form / w | Gruppe | streng |
|---|---|---|--:|--:|
| eigener Adapter | `shipped` | solo / 1,00 | **0,467** | 0,067 |
| geliehen vom Gruppenmitglied | `bestmem` | inline / 1,00 | **0,800** | 0,367 |
| Gruppen-Adapter (gepoolt) | `grp25` | solo / 1,50 | **0,500** | 0,133 |

Geliehen gegen eigen, gepaart bei gleicher Form und gleichem Gewicht: d = 0,5333, t = 4,31, n = 10.
...
Wenn dieses Etikett nicht trifft: die Gruppe `laugh_soft` enthält `childlike_giggle`, `chuckle`, `nervous_giggle`, `snicker`. Der Adapter des stärksten gemessenen Mitglieds — `vb/nervous_giggle` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,900).
