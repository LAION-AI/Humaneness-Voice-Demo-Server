...
| | |
|---|---|
| Adapter | **geliehen**: der Adapter des stärksten Gruppenmitglieds — `vb/panting` |
| Gewicht | **1,00** |
...
| Trefferquote, **Gruppe** (irgendein Mitglied) | **0,067** |
| größengleiche Zufallskontrolle | 0,017 |
| netto über der Zufallskontrolle | 0,050 |
| Genuineness (nur berichtet, kein Tor) | 2,51 |
| WER (Parakeet), absolut | 0,100 |
| WER gepaart gegen die eigene w = 0-Zelle | -0,280 (Grenze +0,104) |
| **Detektor-Boden**, echte Sprache, streng | 0,120 ⛔ *nicht belastbar, n < 30* |

Dieselbe Klasse, die drei Wege nebeneinander (jeweils die beste Zelle unter w ≤ 1,5):

| Weg | Adapter | Form / w | Gruppe | streng |
|---|---|---|--:|--:|
| eigener Adapter | `shipped` | solo / 1,00 | **0,033** | 0,033 |
| geliehen vom Gruppenmitglied | `bestmem` | solo / 1,00 | **0,067** | 0,000 |
| Gruppen-Adapter (gepoolt) | `grp25` | inline / 1,00 | **0,033** | 0,000 |

Geliehen gegen eigen, gepaart bei gleicher Form und gleichem Gewicht: d = 0,0333, t = 0,56, n = 10.

## Warum die Zahl so ist

**Die Grenze ist hier das Messgerät, nicht das Modell.** Der Produktionsdetektor erkennt diese Klasse auf echter Sprache nur zu 0,120. Eine gemessene Trefferquote wesentlich über 0,120 ist deshalb gar nicht erreichbar, wie gut der Adapter auch sein mag. Die Zahl oben ist eine Untergrenze für das, was tatsächlich erzeugt wird.

## Wenn es nicht geht

Wenn dieses Etikett nicht trifft: die Gruppe `breath_fast` enthält `fast_breathing`, `panting`. Der Adapter des stärksten gemessenen Mitglieds — `vb/panting` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,100).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `heavy_breathing` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/heavy_breathing) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `heavy_breathing` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/heavy_breathing) |
| Gruppen-Adapter, volle Dosis — Gruppe `breath_fast` | `breath_fast` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_full/breath_fast) |
| Gruppen-Adapter, 25 % synthetisch — Gruppe `breath_fast` | `breath_fast` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_dose25/breath_fast) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (heavy breathing) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
