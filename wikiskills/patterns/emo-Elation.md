...
`emo/Elation` — emotion. Generated from the measured JSON on 2026-09-02 by `code/build_wikiskills.py`; do not edit by hand, edit the generator or the measurements it reads.

Steering vector key `emo:Elation`. Target metric `emo_pct`, baseline 0.397 over 10 prompts.

## Coefficient block

```yaml
attribute: emo/Elation
steering_key: emo:Elation
target_metric: emo_pct
adapter:
  name: sft3_emotion:Elation
  usable: false
  safe_w: null
  strong_w: null
...
balanced:
  mode: adapter+steer
  lora: {name: "sft3_emotion:Elation", w: 1.0}
  steer:
    - {key: "emo:Elation", alpha: 0.15, taps: top1}   # h20
  steer_branch: cond
  cfg: {g: 1.0}
  measured: {d_target: +0.281, t: 3.72, n_prompts: 10, n_up: 10,
             d_wer_parakeet: +0.008, d_genuineness: -0.294,
             d_blend: -0.111, d_r_burst: -0.026, d_dur_err_abs_s: -0.001}
...
high_effect:
  ...
  steer_branch: cond
  cfg: {g: 1.0}
  measured: {d_target: +0.379, t: 5.80, n_prompts: 10, n_up: 10,
             d_wer_parakeet: +0.076, d_genuineness: +0.461,
             d_blend: -0.133, d_r_burst: -0.035, d_dur_err_abs_s: -0.009}
  beats_random_floor: true
```
...
Mode **`adapter+steer`**. Adapter `sft3_emotion:Elation` at w = 1.0. Steering on the cond branch. 

Target moves **+0.281** (t 3.72, better on 10 of 10 prompts), from 0.397 to 0.678. This clears the matched random-direction floor of -0.020.

| guardrail | baseline | at this point | change |
|---|--:|--:|--:|
| word error (Parakeet) | 0.075 | 0.083 | +0.008 |
| genuineness, raw of 6 | 3.821 | 3.528 | -0.294 |
| burst blend, raw of 10 | 5.134 | 5.022 | -0.111 |
| burst realisation | 0.406 | 0.379 | -0.026 |
| |duration error|, s | 0.063 | 0.062 | -0.001 |

## High effect operating point

Mode **`adapter+steer`**. Adapter `sft3_emotion:Elation` at w = 1.0. Steering on the cond branch. 

Target moves **+0.379** (t 5.80, better on 10 of 10 prompts), from 0.397 to 0.776. This clears the matched random-direction floor of -0.020.

| guardrail | baseline | at this point | change |
|---|--:|--:|--:|
| word error (Parakeet) | 0.075 | 0.151 | +0.076 |
| genuineness, raw of 6 | 3.821 | 4.282 | +0.461 |
| burst blend, raw of 10 | 5.134 | 5.001 | -0.133 |
| burst realisation | 0.406 | 0.371 | -0.035 |
| |duration error|, s | 0.063 | 0.054 | -0.009 |
...
|--:|--:|--:|--:|--:|--:|--:|--:|:--|
| 0.25 | 0.253 | -0.017 | -0.43 | 1/10 | -0.005 | +0.095 | +0.085 | yes |
| 0.5 | 0.250 | -0.020 | -0.52 | 2/10 | +0.009 | +0.166 | -0.025 | yes |
| 0.75 | 0.274 | +0.004 | 0.13 | 2/10 | +0.011 | -0.049 | +0.017 | yes |
| 1.0 | 0.289 | +0.019 | 0.46 | 2/10 | +0.026 | +0.176 | +0.070 | yes |
| 1.25 | 0.261 | -0.010 | -0.27 | 3/10 | +0.003 | +0.079 | +0.026 | yes |
| 1.5 | 0.271 | +0.001 | 0.09 | 3/10 | +0.062 | +0.084 | -0.005 | yes |
...
| pair | interaction | t |
|---|--:|--:|
| adapter × steering | +0.033 | 0.23 |
| adapter × guidance | +0.229 | 1.22 |
| steering × guidance | +0.331 | 1.40 |

Target in SD units at each corner of the 2×2×2 (bits are adapter, steering, guidance):

| 000 | 001 | 010 | 011 | 100 | 101 | 110 | 111 |
|--:|--:|--:|--:|--:|--:|--:|--:|
| 1.207 | 1.047 | 1.780 | 2.042 | 1.022 | 1.182 | 1.719 | 2.119 |
...
* Recipes and every Δ in this page: `combination-study/stats/comb_recommendations.json`, key `emo/Elation`.
* Interaction terms: `combination-study/stats/analysis.json`, `t2`.
* Dose ladder: `lora-dose/coefficients.json`, key `emotion/Elation`.
* Layer ranking behind `topK`: `work_vb/tap_rank.json`.
* Steering vectors: `actforensics/vectors/p3_vectors_ext.npz`, row `emo:Elation`.
