...
> Stand 5. September 2026 · Gruppe `sigh_neg` im 23-Gruppen-Schema · **nicht einzeln gemessen** · eigener Adapter vorhanden: nein
>
> Gruppen-Mitglieder: `displeased_grunt`, `effort_groan`, `exasperated_sigh`, `exhausted_groan`, `frustrated_groan`

## Rezept

Diese Bezeichnung wurde **nicht einzeln gemessen**. Sie gehört zur Gruppe `sigh_neg`; dort sind gemessen: `displeased_grunt`, `exasperated_sigh`, `exhausted_groan`, `frustrated_groan`.


Wer diesen Laut braucht, nimmt den Adapter des stärksten gemessenen Geschwisters, `vb/exhausted_groan`, mit der dort belegten Einstellung — siehe `vb-exhausted_groan.md` (dort gemessen: 0,467 auf Gruppenebene).
...
Wenn dieses Etikett nicht trifft: die Gruppe `sigh_neg` enthält `displeased_grunt`, `exasperated_sigh`, `exhausted_groan`, `frustrated_groan`. Der Adapter des stärksten gemessenen Mitglieds — `vb/exhausted_groan` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,467).
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (effort groan) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
