...
| dieselbe Zelle im **veröffentlichten 23-Gruppen-Schema** | 0,400 — bestes Rezept dort: `bulk_alone` / inline / w = 1,50 |
| Trefferquote, streng | 0,000 — in **keiner** Zelle über null |
| Genuineness | 1,59 |
| WER (Parakeet) | 0,162, gepaart +0,036 gegen die eigene w = 0-Zelle |

**Entscheidung: das neue Rezept ersetzt das alte.** 0,400 gegen 0,280 familienweit, also mehr als eine Seed-Rausch-Standardabweichung (0,068) besser.
...
Bei einer Trefferquote von 0,40 braucht es **5 Kandidaten**, damit mit 90 % Wahrscheinlichkeit mindestens einer den Laut realisiert. Konservativ gerechnet (0,33, eine Seed-Rausch-Standardabweichung abgezogen): **6 Kandidaten**.
...
| Trefferquote, **Gruppe** (irgendein Mitglied) | **0,667** |
| größengleiche Zufallskontrolle | 0,071 |
| netto über der Zufallskontrolle | 0,595 |
| Genuineness (nur berichtet, kein Tor) | 2,21 |
| WER (Parakeet), absolut | 0,086 |
...
|---|---|---|--:|--:|
| eigener Adapter | `shipped` | inline / 1,00 | **0,667** | 0,000 |
| geliehen vom Gruppenmitglied | `bestmem` | inline / 0,50 | **0,500** | 0,000 |
| Gruppen-Adapter (gepoolt) | `grp25` | inline / 1,00 | **0,467** | 0,000 |

Geliehen gegen eigen, gepaart bei gleicher Form und gleichem Gewicht: d = 0,1000, t = 1,15, n = 10.
...
Wenn dieses Etikett nicht trifft: die Gruppe `gasp` enthält `sharp_inhale`, `surprised_gasp`. Der Adapter des stärksten gemessenen Mitglieds — `vb/surprised_gasp` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,800).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `fearful_gasp` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/fearful_gasp) |
| eigener Adapter, nur Erstwahl-Zeilen | `fearful_gasp` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class_top1/fearful_gasp) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `fearful_gasp` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/fearful_gasp) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (fearful gasp) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
