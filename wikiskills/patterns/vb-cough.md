...
| größengleiche Zufallskontrolle | 0,012 |
| netto über der Zufallskontrolle | 0,188 |
| Genuineness (nur berichtet, kein Tor) | 3,30 |
| WER (Parakeet), absolut | 0,360 |
...
|---|---|---|--:|--:|
| eigener Adapter | `shipped` | solo / 0,50 | **0,200** | 0,000 |
| Gruppen-Adapter (gepoolt) | `grpfull` | solo / 1,00 | **0,133** | 0,000 |
...
Wenn dieses Etikett nicht trifft: die Gruppe `throat` enthält `clears_throat`, `coughing`. Der Adapter des stärksten gemessenen Mitglieds — `vb/clears_throat` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,300).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `cough` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/cough) |
| eigener Adapter, nur Erstwahl-Zeilen | `cough` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class_top1/cough) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `cough` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/cough) |
| Gruppen-Adapter, volle Dosis — Gruppe `throat` | `throat` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/groups_full/throat) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (cough) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
