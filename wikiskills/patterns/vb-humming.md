...
| Burst-Adapter | `vbr/humming` bei Gewicht **0.8** |
...
|---|--:|--:|
| Trefferquote, Familie | **0.233** | +0.050 (t +0.8, 4/10) |
...
Grundform: w 0.3: 0.18 · w 0.8: 0.23 · w 1.3: 0.25 · w 1.8: 0.33.

§43 hatte für diese Klasse 0.8 empfohlen und dort 0.333 gemessen.
...
| dieselbe Zelle im **veröffentlichten 23-Gruppen-Schema** | 0,533 — bestes Rezept dort: `bulk` / solo / w = 1,00 |
| Trefferquote, streng | 0,033 — aber in einer anderen Zelle: `shipped`, eingebettet, w = 1,00 |
| Genuineness | 2,67 |
| WER (Parakeet) | 0,273, gepaart -0,020 gegen die eigene w = 0-Zelle |
| Detektor-Boden (echte Sprache) | streng 0,472, Familie 0,611 |

**Entscheidung: das neue Rezept ersetzt das alte.** 0,533 gegen 0,230 familienweit, also mehr als eine Seed-Rausch-Standardabweichung (0,068) besser.
...
Bei einer Trefferquote von 0,53 braucht es **4 Kandidaten**, damit mit 90 % Wahrscheinlichkeit mindestens einer den Laut realisiert. Konservativ gerechnet (0,47, eine Seed-Rausch-Standardabweichung abgezogen): **4 Kandidaten**.
...
| größengleiche Zufallskontrolle | 0,242 |
| netto über der Zufallskontrolle | 0,158 |
| Genuineness (nur berichtet, kein Tor) | 2,67 |
| WER (Parakeet), absolut | 0,273 |
| WER gepaart gegen die eigene w = 0-Zelle | -0,020 (Grenze +0,104) |
| **Detektor-Boden**, echte Sprache, streng | 0,472 |

Dieselbe Klasse, die drei Wege nebeneinander (jeweils die beste Zelle unter w ≤ 1,5):

| Weg | Adapter | Form / w | Gruppe | streng |
|---|---|---|--:|--:|
| eigener Adapter | `percls` | solo / 1,00 | **0,400** | 0,200 |
| geliehen vom Gruppenmitglied | `bestmem` | solo / 1,00 | **0,400** | 0,200 |
| Gruppen-Adapter (gepoolt) | `grp25` | solo / 1,50 | **0,333** | 0,167 |
...
Wenn dieses Etikett nicht trifft: die Gruppe `hum` enthält `purr`, `soft_hum`. Der Adapter des stärksten gemessenen Mitglieds — `vb/humming` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,400).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `humming` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/humming) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `humming` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/humming) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (humming) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
