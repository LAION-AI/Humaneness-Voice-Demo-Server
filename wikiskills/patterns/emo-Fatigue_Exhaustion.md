...
`emo/Fatigue_Exhaustion` — emotion. Generated from the measured JSON on 2026-09-02 by `code/build_wikiskills.py`; do not edit by hand, edit the generator or the measurements it reads.

Steering vector key `emo:Fatigue_Exhaustion`. Target metric `emo_pct`, baseline 0.396 over 10 prompts.

## Coefficient block

```yaml
attribute: emo/Fatigue_Exhaustion
steering_key: emo:Fatigue_Exhaustion
target_metric: emo_pct
adapter:
  name: sft3_emotion:Fatigue_Exhaustion
  usable: false
  safe_w: null
  strong_w: null
...
balanced:
  mode: adapter+cfg
  lora: {name: "sft3_emotion:Fatigue_Exhaustion", w: 1.0}
  steer: []
  cfg: {g: 2.0}
  measured: {d_target: +0.113, t: 1.25, n_prompts: 10, n_up: 5,
             d_wer_parakeet: +0.007, d_genuineness: +0.109,
             d_blend: -0.558, d_r_burst: -0.056, d_dur_err_abs_s: -0.005}
...
Mode **`adapter+cfg`**. Adapter `sft3_emotion:Fatigue_Exhaustion` at w = 1.0. Guidance g = 2.0. 

Target moves **+0.113** (t 1.25, better on 5 of 10 prompts), from 0.396 to 0.508. This clears the matched random-direction floor of -0.033.

| guardrail | baseline | at this point | change |
|---|--:|--:|--:|
| word error (Parakeet) | 0.085 | 0.091 | +0.007 |
| genuineness, raw of 6 | 3.884 | 3.993 | +0.109 |
| burst blend, raw of 10 | 5.108 | 4.551 | -0.558 |
| burst realisation | 0.495 | 0.439 | -0.056 |
| |duration error|, s | 0.051 | 0.046 | -0.005 |
...
|--:|--:|--:|--:|--:|--:|--:|--:|:--|
| 0.25 | 0.681 | -0.030 | -0.85 | 4/10 | -0.022 | -0.044 | -0.039 | yes |
| 0.5 | 0.675 | -0.036 | -0.90 | 5/10 | -0.039 | -0.028 | +0.084 | yes |
| 0.75 | 0.664 | -0.047 | -0.63 | 7/10 | -0.043 | -0.146 | +0.080 | yes |
| 1.0 | 0.693 | -0.018 | -0.31 | 6/10 | +0.011 | +0.016 | +0.008 | yes |
| 1.25 | 0.718 | +0.007 | 0.15 | 6/10 | -0.025 | +0.075 | -0.083 | yes |
| 1.5 | 0.730 | +0.019 | 0.46 | 7/10 | -0.019 | +0.013 | +0.011 | yes |
...
| pair | interaction | t |
|---|--:|--:|
| adapter × steering | +0.005 | 0.02 |
| adapter × guidance | -0.403 | -1.81 |
| steering × guidance | +0.246 | 1.01 |

Target in SD units at each corner of the 2×2×2 (bits are adapter, steering, guidance):

| 000 | 001 | 010 | 011 | 100 | 101 | 110 | 111 |
|--:|--:|--:|--:|--:|--:|--:|--:|
| 1.490 | 1.845 | 1.971 | 2.732 | 1.802 | 1.914 | 2.448 | 2.646 |
...
* Recipes and every Δ in this page: `combination-study/stats/comb_recommendations.json`, key `emo/Fatigue_Exhaustion`.
* Interaction terms: `combination-study/stats/analysis.json`, `t2`.
* Dose ladder: `lora-dose/coefficients.json`, key `emotion/Fatigue_Exhaustion`.
* Layer ranking behind `topK`: `work_vb/tap_rank.json`.
* Steering vectors: `actforensics/vectors/p3_vectors_ext.npz`, row `emo:Fatigue_Exhaustion`.
