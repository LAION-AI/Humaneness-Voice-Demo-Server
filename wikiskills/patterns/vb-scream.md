...
| | |
|---|---|
| Adapter | Gruppen-Adapter, volle DramaBox-Dosis (nur der Adapter, ohne Produktions-Stapel) |
| Gewicht | **1,50** |
...
| Trefferquote, **Gruppe** (irgendein Mitglied) | **0,600** |
| größengleiche Zufallskontrolle | 0,606 |
| netto über der Zufallskontrolle | -0,006 |
| Genuineness (nur berichtet, kein Tor) | 1,27 |
| WER (Parakeet), absolut | 0,148 |
| WER gepaart gegen die eigene w = 0-Zelle | -0,007 (Grenze +0,104) |
| **Detektor-Boden**, echte Sprache, streng | 0,595 |

Dieselbe Klasse, die drei Wege nebeneinander (jeweils die beste Zelle unter w ≤ 1,5):

| Weg | Adapter | Form / w | Gruppe | streng |
|---|---|---|--:|--:|
| eigener Adapter | `percls_alone` | inline / 1,50 | **0,433** | 0,433 |
| geliehen vom Gruppenmitglied | `bestmem_alone` | inline / 1,50 | **0,433** | 0,433 |
| Gruppen-Adapter (gepoolt) | `grpfull_alone` | inline / 1,50 | **0,600** | 0,600 |
...
Wenn dieses Etikett nicht trifft: die Gruppe `scream` enthält `shriek`. Der Adapter des stärksten gemessenen Mitglieds — `vb/scream` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,600).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `scream` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/scream) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `scream` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/scream) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (scream) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
