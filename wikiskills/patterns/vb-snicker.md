...
| Burst-Adapter | `vbr/snicker` bei Gewicht **1.25** |
...
|---|--:|--:|
| Trefferquote, Familie | **0.250** | +0.050 (t +2.0, 3/10) |
...
Grundform: w 0.8: 0.23 · w 1.25: 0.23 · w 1.8: 0.22 · w 2.3: 0.18.

§43 hatte für diese Klasse 1.25 empfohlen und dort 0.300 gemessen.
...
| dieselbe Zelle im **veröffentlichten 23-Gruppen-Schema** | 0,467 — bestes Rezept dort: `bulk` / inline / w = 1,00 |
| Trefferquote, streng | 0,000 — in **keiner** Zelle über null |
| Genuineness | 2,02 |
| WER (Parakeet) | 0,070, gepaart -0,104 gegen die eigene w = 0-Zelle |

**Entscheidung: das neue Rezept ersetzt das alte.** 0,467 gegen 0,250 familienweit, also mehr als eine Seed-Rausch-Standardabweichung (0,068) besser.
...
Bei einer Trefferquote von 0,47 braucht es **4 Kandidaten**, damit mit 90 % Wahrscheinlichkeit mindestens einer den Laut realisiert. Konservativ gerechnet (0,40, eine Seed-Rausch-Standardabweichung abgezogen): **5 Kandidaten**.
...
| Trefferquote, **Gruppe** (irgendein Mitglied) | **0,433** |
| größengleiche Zufallskontrolle | 0,144 |
| netto über der Zufallskontrolle | 0,289 |
| Genuineness (nur berichtet, kein Tor) | 2,02 |
| WER (Parakeet), absolut | 0,070 |
| WER gepaart gegen die eigene w = 0-Zelle | -0,104 (Grenze +0,104) |

Dieselbe Klasse, die drei Wege nebeneinander (jeweils die beste Zelle unter w ≤ 1,5):

| Weg | Adapter | Form / w | Gruppe | streng |
|---|---|---|--:|--:|
| eigener Adapter | `percls` | inline / 1,00 | **0,433** | 0,000 |
| geliehen vom Gruppenmitglied | `bestmem` | inline / 1,00 | **0,300** | 0,000 |
| Gruppen-Adapter (gepoolt) | `grp25` | solo / 1,50 | **0,400** | 0,000 |

Geliehen gegen eigen, gepaart bei gleicher Form und gleichem Gewicht: d = -0,1333, t = -0,94, n = 10.
...
Wenn dieses Etikett nicht trifft: die Gruppe `laugh_soft` enthält `breathy_giggle`, `childlike_giggle`, `chuckle`, `nervous_giggle`. Der Adapter des stärksten gemessenen Mitglieds — `vb/nervous_giggle` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,900).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `snicker` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/snicker) |
| eigener Adapter, nur Erstwahl-Zeilen | `snicker` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class_top1/snicker) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `snicker` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/snicker) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (snicker) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
