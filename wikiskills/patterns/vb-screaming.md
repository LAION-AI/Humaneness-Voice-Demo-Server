...
> Stand 5. September 2026 · Gruppe `scream` im 23-Gruppen-Schema · **nicht einzeln gemessen** · eigener Adapter vorhanden: nein
>
> Gruppen-Mitglieder: `scream`, `screaming`, `screech`, `shout`, `shriek`, `yell`

## Rezept

Diese Bezeichnung wurde **nicht einzeln gemessen**. Sie gehört zur Gruppe `scream`; dort sind gemessen: `scream`, `shriek`.


Wer diesen Laut braucht, nimmt den Adapter des stärksten gemessenen Geschwisters, `vb/scream`, mit der dort belegten Einstellung — siehe `vb-scream.md` (dort gemessen: 0,600 auf Gruppenebene).
...
Wenn dieses Etikett nicht trifft: die Gruppe `scream` enthält `scream`, `shriek`. Der Adapter des stärksten gemessenen Mitglieds — `vb/scream` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,600).
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (screaming) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
