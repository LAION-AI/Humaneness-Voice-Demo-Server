...
| dieselbe Zelle im **veröffentlichten 23-Gruppen-Schema** | 0,100 — bestes Rezept dort: `bulk_top1` / solo / w = 1,50 |
| Trefferquote, streng | 0,067 — aber in einer anderen Zelle: `bulk_top1`, eingebettet, w = 1,50 |
| Genuineness | 2,43 |
| WER (Parakeet) | 0,620, gepaart -0,427 gegen die eigene w = 0-Zelle |

**Entscheidung: das neue Rezept ersetzt das alte.** 0,633 gegen 0,540 familienweit, also mehr als eine Seed-Rausch-Standardabweichung (0,068) besser.
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `cackle` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/cackle) |
| eigener Adapter, nur Erstwahl-Zeilen | `cackle` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class_top1/cackle) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `cackle` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/cackle) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (cackle) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
