...
> Stand 5. September 2026 · Gruppe `sniff` im 23-Gruppen-Schema · **nicht einzeln gemessen** · eigener Adapter vorhanden: nein
>
> Gruppen-Mitglieder: `sniff`, `sniffle`, `snort`

## Rezept

Diese Bezeichnung wurde **nicht einzeln gemessen**. Sie gehört zur Gruppe `sniff`; dort sind gemessen: `sniff`, `snort`.


In dieser Gruppe funktioniert **nichts**: das gemessene Mitglied kommt nicht über 0,000. Für diesen Laut gibt es kein Rezept, das etwas bringt — siehe `vb-sniff.md`.
...
Die Gruppe `sniff` hilft hier nicht: auch ihre gemessenen Mitglieder (`sniff`, `snort`) kommen nicht über 0,000. Für diesen Laut gibt es derzeit **kein funktionierendes Rezept**; den Charakter über die Emotionsanweisung zu steuern ist der ehrliche Rat.
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (sniffle) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
