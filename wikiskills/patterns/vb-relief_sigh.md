...
| Maß | Wert |
|---|--:|
| Trefferquote, **streng** (exakt dieselbe Klasse) | **0,133** |
| Trefferquote, **Gruppe** (irgendein Mitglied) | **0,200** |
| größengleiche Zufallskontrolle | 0,146 |
| netto über der Zufallskontrolle | 0,054 |
| Genuineness (nur berichtet, kein Tor) | 1,62 |
| WER (Parakeet), absolut | 0,193 |
| WER gepaart gegen die eigene w = 0-Zelle | 0,050 (Grenze +0,104) |
| **Detektor-Boden**, echte Sprache, streng | 0,062 |

Dieselbe Klasse, die drei Wege nebeneinander (jeweils die beste Zelle unter w ≤ 1,5):

| Weg | Adapter | Form / w | Gruppe | streng |
|---|---|---|--:|--:|
| eigener Adapter | `percls` | inline / 1,50 | **0,200** | 0,133 |
| geliehen vom Gruppenmitglied | `bestmem` | inline / 0,50 | **0,067** | 0,000 |
| Gruppen-Adapter (gepoolt) | `grpfull` | inline / 0,50 | **0,067** | 0,000 |

Geliehen gegen eigen, gepaart bei gleicher Form und gleichem Gewicht: d = 0,0333, t = 0,56, n = 10.

## Warum die Zahl so ist

**Die Grenze ist hier das Messgerät, nicht das Modell.** Der Produktionsdetektor erkennt diese Klasse auf echter Sprache nur zu 0,062. Eine gemessene Trefferquote wesentlich über 0,062 ist deshalb gar nicht erreichbar, wie gut der Adapter auch sein mag. Die Zahl oben ist eine Untergrenze für das, was tatsächlich erzeugt wird.

## Wenn es nicht geht

Wenn dieses Etikett nicht trifft: die Gruppe `sigh_pos` enthält `wistful_sigh`. Der Adapter des stärksten gemessenen Mitglieds — `vb/wistful_sigh` — ist für die ganze Gruppe eine belegte Alternative (dort gemessen: 0,200).
...
|---|---|---|
| **eigener Adapter, nachtrainiert** | `relief_sigh` | [`laion/moss-va-sft3-vocal-burst-lora-adapters-v2`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters-v2/tree/main/per_class/relief_sigh) |
| **ausgeliefert** (bisheriger Satz, Rückfallvariante) | `relief_sigh` | [`laion/moss-va-sft3-vocal-burst-lora-adapters`](https://huggingface.co/laion/moss-va-sft3-vocal-burst-lora-adapters/tree/main/adapters/relief_sigh) |
...
* Der Burst ist **eine eigene Klammer** zwischen den Wörtern — `(clearly amused) … (relief sigh) …`. Innerhalb einer Regieanweisung genannt entsteht **kein** Laut.
